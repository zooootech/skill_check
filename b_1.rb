...
m.times do |i|
  head = b[i] - 1  # 最終的に添字として用いるため、-1 をしている
  tail = a[i] + b[i] - 2  # 最終的に添字として用いるため、a[i] + b[i] - 1 から -1 をしている
  # head と tail を最終的に添字として用いて配列の要素を指定し、その席が埋まった処理を施す

  range = (head..tail).map { |j| j % n }  # 座席 n と座席 1 が隣り合っている為、j を n で割った剰余を要素に持つ配列を生成

  # 座席 b_i から連続する a_i 席に相当する table の各要素の和を計算し、それが 0 であるかどうかを確認することで、そのグループが座れるかどうかを判定し、座れるのであれば埋めた席を 0 から 1 に変更する
  # range.sum { |j| table[j] } でtable の各要素の和を計算し、その後 zero? で 0 かどうかを判定する
  # sum にブロックが与えられた場合、加算する前に各要素にブロックが適用される
