...
# (自分が出す指の本数 / 5) をすることで m 回の勝負におけるパーが出せる最大の数が分かります。
# 例えば、 m = 18 の場合、18回の勝負におけるパーが出せる回数は、 18 / 5 == 3 なので 3 回となります。
# downto(min) は self から min まで 1 ずつ減らしながらブロックを繰り返し実行します。
(m / 5).downto(0) do |pa|
  # (自分が出せる指の本数 - 5 × パーを出した回数) / 2 をすることで m 回の勝負におけるチョキが出せる最大の数が分かります。
  ch = (m - 5 * pa) / 2
  # じゃんけんを行う回数から、パーを出した回数とチョキを出した回数を引き、グーを出した回数を求めます。
  gu = n - pa - ch

  # negative? は self が 0 未満の場合に true を返します。
  # もしグーが 0 未満の場合、 break によりループ処理を抜けます。
  # なぜ処理を抜けるのかというと、 gu がマイナスの値になるということは、そのターンでのパーとチョキの組み合わせにおける、パーとチョキを出した合計回数が n（じゃんけんの回数） を上回ってしまっているからです。
  break if gu.negative?
  # next により、自分が出せる指の本数が、実際に自分が出す指の本数 (5 * pa + 2 * ch) と一致しない場合の処理をスキップします。
  # なせスキップするのかというと、このじゃんけんゲームにおいて、指定された指の本数 (m本) になるようにグー、チョキ、パーを組み合わせて出さなければならないという最低限のルールがあるからです。
  # ちなみに、繰り返しが入れ子になっている場合は next が実行された一番内側の繰り返しに関して次の繰り返しへ進みます。
  next if m != 5 * pa + 2 * ch

  # min の引数として、「相手が出すチョキの回数と自分が出すグーの回数」、「相手が出すパーの回数と自分が出すチョキの回数」、「相手が出すグーの回数と自分が出すチョキの回数」を渡し、グー出した時、チョキ出した時、パー出した時それぞれの最大勝利数を求めます。
  # max の引数として、「現状の win_max の値」と、「グー、チョキ、パーそれぞれの最大勝利数を足したもの」を比較し、値が大きい方をwin_maxに代入します。
